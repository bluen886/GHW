...
def create_mask_from_points(points, img_h, img_w):
    ...
    points = np.asarray(points, dtype=np.int32).copy()
    points[:, 0] = np.clip(points[:, 0], 0, img_w - 1)
    points[:, 1] = np.clip(points[:, 1], 0, img_h - 1)

    mask_img = Image.new('L', (img_w, img_h), 0)
    draw = ImageDraw.Draw(mask_img)
    draw.polygon([tuple(pt) for pt in points.tolist()], outline=255, fill=255)
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    """
    loss = torch.tensor(0.0, device=foreground_img.device, dtype=foreground_img.dtype)
    ...
    fg_laplacian = F.conv2d(foreground_img, lap_kernel, padding=1, groups=channels)
    blended_laplacian = F.conv2d(blended_img, lap_kernel, padding=1, groups=channels)

    fg_mask_bool = (foreground_mask > 0.5).expand(-1, channels, -1, -1)
    bg_mask_bool = (background_mask > 0.5).expand(-1, channels, -1, -1)

    fg_values = fg_laplacian[fg_mask_bool]
    blended_values = blended_laplacian[bg_mask_bool]

    if fg_values.numel() == 0 or blended_values.numel() == 0:
        return loss

    if fg_values.numel() != blended_values.numel():
        valid_numel = min(fg_values.numel(), blended_values.numel())
        fg_values = fg_values[:valid_numel]
        blended_values = blended_values[:valid_numel]

    loss = torch.mean((blended_values - fg_values) ** 2)
