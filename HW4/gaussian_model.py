...
class GaussianParameters:
    ...
    scales: torch.Tensor      # (N, 3) Positive scales
...
class GaussianModel(nn.Module):
    ...
    def _init_rotations(self) -> None:
        ...
        initial_rotations = torch.zeros((self.n_points, 4), dtype=torch.float32)
    ...
    def _init_scales(self, points3D_xyz: torch.Tensor) -> None:
        ...
        points3D_xyz = torch.as_tensor(points3D_xyz, dtype=torch.float32)
        ...
        mean_dists = mean_dists.clamp(
            0.2 * median_dist,
            3.0 * median_dist
        )  # Prevent infinite/extreme scales
    ...
    def compute_covariance(self) -> torch.Tensor:
        ...
        # Convert log-scales to positive world-space scales.
        scales = torch.exp(self.scales)
        S = torch.diag_embed(scales)  # (N, 3, 3)

        # Paper Eq. (6): Sigma = R S S^T R^T.
        # Equivalently, with M = R S, Sigma = M M^T.
        M = torch.bmm(R, S)
